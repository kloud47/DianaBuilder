...
const AllSubaccounts = async ({ params, searchParams }: Props) => {
    ...
    return (
        <AlertDialog>
            <div className='flex flex-col overflow-x-hidden'>
                <CreateSubaccount user={user} id={params.agencyId} className='w-[300px] float-end' />
                <Command>
                                    ...
                                    <CommandItem className='h-32 !bg-background my-2 text-primary border-[1px] border-border p-4 rounded-lg hover:!bg-background cursor-pointer transition-all'>
                                        <Link href={`/subaccount/${subaccount.id}`} className="flex gap-4 w-full h-full">
                                            <div className='relative w-32'>
                                                <Image src={subaccount.subAccountLogo} alt="subaccount logo" fill 
